  ...
  const fetchQuestionPapers = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3001/api/questionpapers"
      );
  ...
  const handleDeleteQuestionPaper = async (id) => {
    try {
      const response = await axios.delete(
        `http://localhost:3001/api/questionpapers/${id}`
      );
      console.log(response.data.message); // Assuming the API returns a message upon successful deletion
